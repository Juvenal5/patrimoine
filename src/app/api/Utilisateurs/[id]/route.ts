...
// // ── GET /api/Utilisateurs/[id] ────────────────────────────────────────────────
// // Retourne l'utilisateur avec ses affectations en cours et ses biens
...
//         updatedAt:    true,
//         departementId: true,
//         departement:  { select: { id: true, nom: true, description: true } },
//         affectations: {
...
//   try {
//     const body = await req.json();
//     const { nom, prenom, email, telephone, role, actif, departementId, password } = body;
...
//     const existing = await prisma.user.findFirst({
//       where: { email: email.trim().toLowerCase(), NOT: { id: params.id } },
//     });
//     if (existing) {
...
// // ── DELETE /api/Utilisateurs/[id] ─────────────────────────────────────────────
// // Désactive l'utilisateur si des affectations existent, sinon supprime
// export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
//   try {
//     const { searchParams } = new URL(req.url);
//     const force = searchParams.get("force") === "true";

//     const affCount = await prisma.affectation.count({ where: { userId: params.id } });
...
//       return NextResponse.json({
//         deactivated: true,
//         message: `L'utilisateur a ${affCount} affectation(s) — il a été désactivé plutôt que supprimé.`,
//         user,
//       });
//     }

//     // Suppression réelle (cascade sur historiques d'abord)
//     await prisma.historique.deleteMany({ where: { userId: params.id } });
