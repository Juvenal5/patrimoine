...
const Ico = {
  ...
  ShieldOff:  () => <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><line x1="4.5" y1="4.5" x2="19.5" y2="19.5"/></svg>,
...
function AccessDenied({ userRole, onBack }: { userRole?: string; onBack: () => void }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 24, background: "#0f1824",
        border: "1px solid rgba(239,68,68,0.2)",
        boxShadow: "0 32px 80px rgba(0,0,0,0.5), 0 0 60px rgba(239,68,68,0.05)",
      }}>
        {/* Icône */}
        <div style={{
        ...
        <p style={{ fontSize: 14, color: "#64748b", lineHeight: 1.7, margin: "0 0 24px" }}>
          ...
          <span style={{ color: "#a78bfa", fontWeight: 600 }}>administrateurs</span>.
          {userRole && (
            <>
        ...
        <div style={{
          ...
          fontSize: 12, color: "#94a3b8", lineHeight: 1.65, textAlign: "left",
        }}>
          <span style={{ color: "#a78bfa", fontWeight: 600 }}>Besoin d'accès ?</span><br />
          Contactez un administrateur de la plateforme pour obtenir les droits nécessaires.
        </div>
        ...
        >
          <Ico.ArrowLeft /> Retour au tableau de bord
        </button>
      </div>
    </div>
  );
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ position: "relative" }}>
                ...
                <div style={{ position: "absolute", bottom: -2, right: -2, width: 14, height: 14, borderRadius: "50%", background: enLigne ? "#22c55e" : "#374151", border: "2px solid #0a1120", boxShadow: enLigne ? "0 0 8px rgba(34,197,94,0.5)" : "none", animation: enLigne ? "onlinePulse 2.5s ease-in-out infinite" : "none" }} />
              </div>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: role.bg, color: role.text }}>
                    <span style={{ width: 5, height: 5, borderRadius: "50%", background: role.dot }} />{role.label}
                  </span>
...
export default function UtilisateursPage() {
  ...
  // ══════════════════════════════════════════════════
  //  GUARD — vérification session + rôle ADMIN
  // ══════════════════════════════════════════════════
  const { data: session, status } = useSession();
  const userRole = (session?.user as any)?.role as string | undefined;
  const isAdmin  = userRole === "ADMIN";

  // Pendant le chargement de la session → loader
  if (status === "loading") return <SessionLoader />;

  // Non connecté → NextAuth redirige vers /auth via le middleware,
  // mais on garde ce fallback côté client pour les edge cases
  if (status === "unauthenticated") {
    router.push("/auth");
    return <SessionLoader />;
  }

  // Connecté mais pas admin → écran de refus
  if (!isAdmin) {
    return (
      <AccessDenied
        userRole={userRole}
  ...
  // ══════════════════════════════════════════════════
  //  Contenu de la page — accès ADMIN confirmé
  // ══════════════════════════════════════════════════
  return <UtilisateursPageContent />;
}

// ─── Contenu réel de la page (isolé pour éviter les hooks conditionnels) ──────
...
function UtilisateursPageContent() {
  ...
  const fetchOnlineStatuses = useCallback(async () => {
    ...
    try {
      ...
      arr.forEach(u => { statuses[u.id] = u.enLigne; updatedAts[u.id] = u.updatedAt; });
  ...
  return (
    <div style={{ minHeight: "100%", padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20, fontFamily: "'DM Sans','Inter',sans-serif" }}>
                    ...
                    {loading ? <SkeletonRows cols={9} /> : paginated.length === 0 ? (
                      ...
                      return (
                        <tr key={u.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)", transition: "background 0.15s" }}
                          ...
                          <td style={{ padding: "12px 16px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <div style={{ position: "relative", flexShrink: 0 }}>
                                ...
                                <div style={{ position: "absolute", bottom: -1, right: -1, width: 10, height: 10, borderRadius: "50%", background: enLigne ? "#22c55e" : "#374151", border: "1.5px solid #0f1824", boxShadow: enLigne ? "0 0 5px rgba(34,197,94,0.5)" : "none", animation: enLigne ? "onlinePulse 2.5s ease-in-out infinite" : "none" }} />
                              </div>
                              <div>
                                <p style={{ fontSize: 13, fontWeight: 600, color: "#fff", margin: 0, lineHeight: 1 }}><Highlight text={`${u.prenom} ${u.nom}`} query={search} /></p>
                                <p style={{ fontSize: 10, color: "#475569", margin: "2px 0 0" }}><Highlight text={u.telephone || "—"} query={search} /></p>
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: "12px 16px" }}><span style={{ fontSize: 12, color: "#94a3b8", fontFamily: "monospace" }}><Highlight text={u.email} query={search} /></span></td>
                          <td style={{ padding: "12px 16px" }}><span style={{ fontSize: 12, color: "#94a3b8" }}><Highlight text={u.departement?.nom || "—"} query={search} /></span></td>
                          <td style={{ padding: "12px 16px" }}>
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: role.bg, color: role.text }}>
                              <span style={{ width: 5, height: 5, borderRadius: "50%", background: role.dot, flexShrink: 0 }} />{role.label}
                            </span>
                          </td>
                          <td style={{ padding: "12px 16px" }}><OnlineIndicator enLigne={enLigne} showLabel updatedAt={onlineUpdatedAt[u.id]} /></td>
                          <td style={{ padding: "12px 16px" }}><span style={{ fontSize: 12, fontWeight: 600, color: (u._count?.affectations ?? 0) > 0 ? "#38bdf8" : "#334155" }}>{u._count?.affectations ?? 0}</span></td>
                          <td style={{ padding: "12px 16px" }}>
                          ...
                          <td style={{ padding: "12px 16px" }}><span style={{ fontSize: 11, color: "#475569" }}>{fmtDate(u.createdAt)}</span></td>
                          <td style={{ padding: "12px 16px" }}>
                            <div style={{ display: "flex", gap: 2 }}>
                              <button title="Voir détail" onClick={() => openDetail(u)} style={{ padding: 6, borderRadius: 6, background: "none", border: "none", color: "#475569", cursor: "pointer" }}>{detailLoading ? <Ico.Spinner /> : <Ico.Info />}</button>
                  ...
                  {paginated.map(u => {
                    ...
                    return (
                      <div key={u.id} style={{ borderRadius: 16, padding: 20, background: "#0f1824", border: `1px solid ${enLigne ? "rgba(34,197,94,0.2)" : !u.actif ? "rgba(239,68,68,0.15)" : "rgba(255,255,255,0.06)"}`, display: "flex", flexDirection: "column", gap: 12, position: "relative", transition: "border-color 0.2s" }}>
                        <div style={{ position: "absolute", top: 12, right: 12 }}><OnlineIndicator enLigne={enLigne} showLabel /></div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, color: "#64748b" }}><Ico.Mail /><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.email}</span></div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                          <span style={{ fontSize: 11, fontWeight: 600, color: (u._count?.affectations ?? 0) > 0 ? "#38bdf8" : "#334155" }}>{u._count?.affectations ?? 0} affectation{(u._count?.affectations ?? 0) !== 1 ? "s" : ""}</span>
      ...
      {detailUser && (
        <DetailPanel user={detailUser} onClose={() => setDetailUser(null)} onEdit={() => openEdit(detailUser)} enLigne={!!onlineStatuses[detailUser.id]} />
      )}
    </div>
  );
}
