...
// ⚠️  npm install qrcode.react
import dynamic from "next/dynamic";

const QRCodeSVG = dynamic(
  () => import("qrcode.react").then(mod => mod.QRCodeSVG),
  { ssr: false, loading: () => <div style={{ width: 28, height: 28, background: "rgba(255,255,255,0.05)", borderRadius: 4 }} /> }
);;
...
const EMPTY_FORM: FormData = {
  ...
  serial: "", departementId: "", dateAchat: "", valeur: "", status: "actif",
...
function mapApiToBien(api: any): Bien {
  ...
  return {
    ...
    responsable,
    dateAchat: api.dateAcquisition ? new Date(api.dateAcquisition).toISOString().split("T")[0] : "",
    valeur: api.valeurAchat ?? 0,
    status: (api.etat as Status) || "actif",
    localisation: api.localisation || "",
    garantie: api.garantieFin ? new Date(api.garantieFin).toISOString().split("T")[0] : "",
...
function buildQrData(b: Bien): string {
  return [
    `CODE:${b.id}`,
    `NOM:${b.designation}`,
    `CAT:${b.categorie}`,
    ...
    `ACHAT:${b.dateAchat}`,
    `VALEUR:${b.valeur} XOF`,
    `ETAT:${STATUS_MAP[b.status].label}`,
    `GARANTIE:${b.garantie || "—"}`,
    `RESP:${b.responsable || "—"}`,
  ].join("\n");
}
...
const Ico = {
  ...
  Qr: () => (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <line x1="14" y1="14" x2="14.01" y2="14"/><line x1="17" y1="14" x2="17.01" y2="14"/>
      <line x1="20" y1="14" x2="20.01" y2="14"/><line x1="14" y1="17" x2="14.01" y2="17"/>
      <line x1="17" y1="17" x2="17.01" y2="17"/><line x1="20" y1="20" x2="20.01" y2="20"/>
    </svg>
  ),
  Print: () => (
...
function ExpandedRow({ bien }: { bien: Bien }) {
  ...
  return (
    <tr>
      <td colSpan={11} style={{ padding: 0, background: "rgba(14,165,233,0.03)", borderBottom: "1px solid rgba(14,165,233,0.12)" }}>
...
function QrModal({ bien, onClose }: { bien: Bien; onClose: () => void }) {
  const qrData  = buildQrData(bien);
  ...
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(`
      <!DOCTYPE html><html><head><title>QR – ${bien.id}</title>
      <style>
        body { margin:0; display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:80vh; font-family:monospace; background:#fff; }
        .card { border:1.5px solid #e2e8f0; border-radius:12px; padding:20px 24px; text-align:center; max-width:200px; }
        h2 { font-size:13px; font-weight:700; margin:12px 0 2px; color:#0f172a; }
        p  { font-size:10px; color:#64748b; margin:0; }
        .meta { margin-top:12px; text-align:left; font-size:10px; color:#475569; line-height:1.8; }
        .meta strong { color:#0f172a; }
        @media print { @page { margin:10mm; } }
      </style></head><body>
      <div class="card">
        ${svgStr}
        <h2>${bien.designation}</h2>
        <p>${bien.id}</p>
        <div class="meta">
          <strong>Catégorie :</strong> ${bien.categorie}<br/>
          <strong>Département :</strong> ${bien.departement}<br/>
          <strong>N° Série :</strong> ${bien.serial || "—"}<br/>
          <strong>Valeur :</strong> ${fmt(bien.valeur)}<br/>
          <strong>Statut :</strong> ${s.label}
        </div>
      </div>
  ...
  const fields = [
    ...
    ["N° Série",    bien.serial   || "—"],
    ["Localisation",bien.localisation || "—"],
    ["Date achat",  fmtDate(bien.dateAchat)],
    ["Fin garantie",fmtDate(bien.garantie)],
    ["Responsable", bien.responsable || "—"],
    ["Valeur",      fmt(bien.valeur)],
  ];
  ...
  return (
    <>
      <div className="fixed inset-0 z-40" style={{ background: "rgba(0,0,0,0.72)", backdropFilter: "blur(6px)" }} onClick={onClose} />
      <div className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex flex-col rounded-3xl overflow-hidden"
        style={{ maxWidth: 420, maxHeight: 640, background: "linear-gradient(160deg,#0f1e2e 0%,#0a1120 100%)", border: "1px solid rgba(14,165,233,0.25)", boxShadow: "0 40px 100px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.06)" }}>

        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-3">
            <div className="w-6 h-6 rounded-xl flex items-center justify-center" style={{ background: "rgba(14,165,233,0.15)", color: "#38bdf8" }}>
              <Ico.Qr />
            </div>
            <div>
              <h3 className="text-sm font-bold text-white leading-none">Code QR du bien</h3>
              <p className="text-[11px] text-slate-500 mt-0.5 font-mono">{bien.id}</p>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center px-6 py-6 gap-4">
          {/* White card with QR */}
          <div ref={svgRef} className="relative rounded-2xl p-4 flex flex-col items-center gap-2"
            style={{ background: "#fff", boxShadow: "0 8px 30px rgba(0,0,0,0.4)", minWidth: 200, minHeight:50 }}>
            {/* Corner decorators */}
            {["top-0 left-0","top-0 right-0","bottom-0 left-0","bottom-0 right-0"].map((pos,i) => (
              <span key={i} className={`absolute ${pos} w-4 h-4`}
                style={{ borderColor: "#0ea5e9", borderWidth: 0,
                  ...(i===0 ? {borderTopWidth:2,borderLeftWidth:2,borderTopLeftRadius:8}    : {}),
                  ...(i===1 ? {borderTopWidth:2,borderRightWidth:2,borderTopRightRadius:8}  : {}),
                  ...(i===2 ? {borderBottomWidth:2,borderLeftWidth:2,borderBottomLeftRadius:8}  : {}),
                  ...(i===3 ? {borderBottomWidth:2,borderRightWidth:2,borderBottomRightRadius:8} : {}),
                }} />
            ))}
            <QRCodeSVG
              value={qrData}
            ...
            <div className="text-center">
              <p className="text-[13px] font-bold text-gray-800 leading-snug">{bien.designation}</p>
              <p className="text-[10px] font-mono text-gray-400 mt-0.5">{bien.id}</p>
          ...
          <div className="w-full grid grid-cols-2 gap-2">
            {fields.map(([k, v]) => (
              <div key={k} className="rounded-xl px-3 py-2" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <p className="text-[9px] uppercase tracking-wider text-slate-600 font-semibold">{k}</p>
                <p className="text-[11px] text-slate-300 font-medium mt-0.5 truncate">{v}</p>
              </div>
            ))}
          </div>
          ...
          <p className="text-[10px] text-slate-700 text-center px-4">
            Ce code QR contient toutes les informations du bien. Scannez-le pour les afficher instantanément.
          </p>
        </div>
        ...
        <div className="flex gap-2 px-6 pb-5">
          <button onClick={handleDownloadSvg}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold text-slate-400 hover:text-white transition-all"
            style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <Ico.Export /> Télécharger SVG
          </button>
          <button onClick={handlePrint}
            ...
            style={{ background: "linear-gradient(135deg,#0891b2,#0e7490)", boxShadow: "0 4px 16px rgba(8,145,178,0.25)" }}>
            <Ico.Print /> Imprimer
          </button>
        </div>
      </div>
...
function DetailPanel({ bien, onClose, onEdit, onQr }: { bien: Bien; onClose: () => void; onEdit: () => void; onQr: () => void }) {
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === "Escape") onClose(); };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full z-50 flex flex-col overflow-hidden"
        ...
        <div className="px-5 py-4 shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-start justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              {/* QR button in detail panel */}
              <button onClick={onQr} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-sky-400 hover:text-white transition-all"
        ...
        {/* Mini QR preview */}
        <div className="mx-5 mt-4 rounded-xl p-4 flex items-center gap-4 shrink-0"
          style={{ background: "rgba(14,165,233,0.05)", border: "1px solid rgba(14,165,233,0.12)" }}>
          <div className="rounded-xl p-2 shrink-0" style={{ background: "#fff" }}>
            <QRCodeSVG value={buildQrData(bien)} size={56} bgColor="#ffffff" fgColor="#0a1219" level="M" includeMargin={false} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-wider text-sky-600 font-semibold">Code QR intégré</p>
            <p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed">Toutes les informations de ce bien sont encodées dans ce QR.</p>
            <button onClick={onQr} className="mt-1.5 text-[10px] font-semibold text-sky-400 hover:text-sky-300 transition-colors underline underline-offset-2">
              Voir en plein écran →
            </button>
          </div>
        </div>
        ...
        <div className="px-5 py-3 flex items-center gap-2 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <Ico.Kbd /><span className="text-[10px] text-slate-700">Appuyez sur <kbd className="text-slate-500">Echap</kbd> pour fermer</span>
        </div>
      </div>
    </>
  );
...
function BienDrawer({ mode, initial, departements, onClose, onSave, saving }: {
  ...
  // Live QR preview data
  const liveQrData = [
    `CODE:${form.codeInventaire || "AUTO"}`,
    `NOM:${form.designation || "—"}`,
    `CAT:${form.categorie}`,
    `MARQUE:${form.marque || "—"}`,
    `SERIE:${form.serial || "—"}`,
    `DEPT:${currentDept?.nom || "—"}`,
    `LIEU:${form.localisation || "—"}`,
    `ACHAT:${form.dateAchat || "—"}`,
    `VALEUR:${form.valeur ? `${form.valeur} XOF` : "—"}`,
    `ETAT:${STATUS_MAP[form.status].label}`,
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {/* QR preview live */}
              {form.designation && (
                <div className="rounded-xl p-4 mt-2" style={{ background: "rgba(14,165,233,0.05)", border: "1px solid rgba(14,165,233,0.15)" }}>
                  <p className="text-[11px] font-semibold text-sky-400 uppercase tracking-wider mb-3">Aperçu du Code QR</p>
                  <div className="flex items-center gap-4">
                    <div className="rounded-xl p-2 shrink-0" style={{ background: "#fff" }}>
                      <QRCodeSVG value={liveQrData} size={72} bgColor="#ffffff" fgColor="#0a1219" level="M" includeMargin={false} />
                    </div>
                  ...
                  <p className="text-[10px] text-slate-600 mt-3">Le QR code sera automatiquement généré et lié au bien après enregistrement.</p>
                </div>
              )}
            </div>
          )}
...
export default function BiensPage() {
  ...
  const [qrTarget, setQrTarget]         = useState<Bien | null>(null);   // ← NEW
  ...
  const fetchBiens = useCallback(async () => {
    try {
      ...
      setBiens((Array.isArray(data) ? data : []).map(mapApiToBien));
    } catch {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (qrTarget)    { setQrTarget(null);    return; }
      ...
      if (e.key === "n" && !formMode && !deleteTarget && !detailBien && !qrTarget) {
    ...
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [detailBien, formMode, deleteTarget, expandedId, qrTarget]);
  ...
  const handleSave = async (data: FormData) => {
    ...
    try {
      const payload = {
        ...
        nom: data.designation, type: data.marque || null, categorie: data.categorie || null,
        numeroSerie: data.serial || null, departementId: data.departementId || null,
        dateAcquisition: data.dateAchat || null, valeurAchat: data.valeur ? parseFloat(data.valeur) : null,
        garantieFin: data.garantie || null, etat: data.status,
        localisation: data.localisation || null, description: data.description || null,
      };
      if (formMode === "create") {
  ...
  const openQr       = (b: Bien) => { setQrTarget(b); setDetailBien(null); };  // ← NEW
  ...
  return (
    <div className="min-h-full px-7 py-6 space-y-5" style={{ fontFamily: "'DM Sans','Inter',sans-serif" }}>
            ...
            <table className="w-full text-left" style={{ background: "#0f1824" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <th className="px-4 py-3 w-16">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-600">QR</span>
                  </th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={11} className="px-4 py-16 text-center text-sm text-slate-600">Aucun bien ne correspond à votre recherche.</td>
                  </tr>
                ) : paginated.map((b, i) => {
                  ...
                  return (
                    <React.Fragment key={b.internalId}>
                      <tr
                        ...
                        {/* ── Mini QR thumbnail ── */}
                        <td className="px-4 py-3">
                          <button onClick={() => openQr(b)} title="Voir le QR code"
                            className="rounded-lg p-1 hover:scale-110 transition-transform"
                            style={{ background: "#fff", display: "inline-block" }}>
                            <QRCodeSVG value={buildQrData(b)} size={28} bgColor="#ffffff" fgColor="#0a1219" level="L" includeMargin={false} />
                          </button>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {/* ── QR button in row actions ── */}
                            <button title="Code QR"       onClick={() => openQr(b)}        className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"><Ico.Qr /></button>
            ...
            <div className="grid grid-cols-3 gap-4">
              {paginated.length === 0 ? (
                ...
                <div key={b.internalId}
                  ...
                  {/* ── QR thumbnail in grid card ── */}
                  <button onClick={() => openQr(b)} title="Voir le QR code"
                    className="self-center rounded-xl p-2 hover:scale-105 transition-transform"
                    style={{ background: "#fff", opacity: 0.85 }}>
                    <QRCodeSVG value={buildQrData(b)} size={52} bgColor="#ffffff" fgColor="#0a1219" level="L" includeMargin={false} />
                  </button>
                  ...
                  <div className="flex items-center justify-between pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => openQr(b)}        className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"><Ico.Qr /></button>
      ...
      {detailBien && (
        <DetailPanel bien={detailBien} onClose={() => setDetailBien(null)} onEdit={() => openEdit(detailBien)} onQr={() => openQr(detailBien)} />
      )}
      {/* ── QR Modal ── */}
      {qrTarget && (
        <QrModal bien={qrTarget} onClose={() => setQrTarget(null)} />
      )}
    </div>
  );
}
